...
<body>
	<div class="container-fluid"
		...
		<main class="container text-center pt-1"
			style="background-color: rgba(255, 255, 255, 0.7);">
			<h2><b style="font-family: 'Nanum Myeongjo', serif;">소설</b></h2><br/>
							...
							<td class="align-middle text-center" colspan="5">
								...
								<c:if test="${bookCategoryList.currentPage > 1}">
									<button 
										...
										type="button" 
										title="첫 페이지로 이동합니다."
										onclick="location.href='list.jsp?list=Novel&currentPage=1'"
									>처음</button>
								</c:if>
								<c:if test="${bookCategoryList.currentPage <= 1}">
									<button 
								...
								<c:if test="${bookCategoryList.startPage > 1}">
									<button
										...
										onclick="location.href='list.jsp?list=Novel&currentPage=${bookCategoryList.startPage - 1}'"
									>이전</button>
								</c:if>
								<c:if test="${bookCategoryList.startPage <= 1}">
									<button
								...
								<c:if test="${bookCategoryList.currentPage != i}">
									<button
										class="button button1"
										type="button"
										onclick="location.href='list.jsp?list=Novel&currentPage=${i}'"
									>${i}</button>
								</c:if>
								...
								<c:if test="${bookCategoryList.endPage < bookCategoryList.totalPage}">
									<button
										...
										onclick="location.href='list.jsp?list=Novel&currentPage=${bookCategoryList.endPage + 1}'"
									>다음</button>
								</c:if>
								...
								<c:if test="${bookCategoryList.currentPage < bookCategoryList.totalPage}">
									<button
										...
										onclick="location.href='list.jsp?list=Novel&currentPage=${bookCategoryList.totalPage}'"
									>마지막</button>
								</c:if>
							</td>
					 	 </tr>	
